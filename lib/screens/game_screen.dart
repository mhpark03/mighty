...
class _GameScreenState extends State<GameScreen> {
          ...
          Container(
            ...
            child: Text(
              bestOptimal >= currentOptimal + 3
                  ? l10n.suitChangeRecommend(_getSuitSymbol(bestSuit!), bestOptimal - currentOptimal)
                  : l10n.suitKeepRecommend(_getSuitSymbol(bestSuit!), bestOptimal - currentOptimal),
              style: TextStyle(
  ...
  String? _describeTrick(Trick trick, GameState state, AppLocalizations l10n, Set<String> playedCards, {bool isAutoPlay = false}) {
    ...
    if (trick.trickNumber == 10) {
      ...
      // 키티(바닥패)에 버린 점수카드는 공격팀 점수에 포함
      attackPoints += state.kitty.where((c) => c.isPointCard).length;
  ...
  String? _getBeginnerAdvice(Trick trick, GameState state, AppLocalizations l10n, Set<String> playedCards) {
    ...
    if (!humanIsLeader && !attackWins && isAttack(0)) {
      if (!humanCard.isJoker && humanCard.isPointCard) {
        // 리드 무늬를 따라갈 수 없어서 어쩔 수 없이 낸 경우는 제외
        // (보유 무늬가 없으면 아무 카드나 내야 함 → 점수카드밖에 없을 수 있음)
        // 간단한 휴리스틱: 리드 무늬를 따라간 경우에만 조언 (선택의 여지가 있었음)
  ...
  String? _describeLeadFromIntent(Trick trick, GameState state, AppLocalizations l10n) {
    ...
    switch (intent) {
      ...
      case LeadIntent.defenseLowCard:
        ...
        if (trick.leadPlayerId == state.declarerId || trick.leadPlayerId == state.friendId) {
          final attackWon = trick.winnerId != null &&
              (trick.winnerId == state.declarerId || trick.winnerId == state.friendId);
